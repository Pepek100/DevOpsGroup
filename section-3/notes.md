...
## What is CPU usage top , free -a , -memory

## monitoring alert (Prometheus, Grafana, Dynatrace)

## look at the logs enter command and /var/log

## check if the server is running
## check if the docker is running - docker ps , docker logs
## for Apache server system status httpd
## check if the tomcat is running ps aux | grep tomcat 
## check if the port is correct - netstat -tulpn
## I use commnad -  ping 
...
## Verify that the application is working after the incident. RCA- root cause analysis will be performed where the problem occurred.
